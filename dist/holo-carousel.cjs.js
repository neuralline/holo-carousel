...
var ManageIO = function ManageIO(virtual, shadow) {
  ...
  if (virtual.io.enabled) {
    ...
    'resize', function () {
      cyre.cyre.call('refresh carousel', {
        virtual: virtual,
        shadow: shadow
      });
    }, false);
...
var Holo = function () {
  cyre.cyre.on('refresh carousel', function (state) {
    ...
    return _holo[virtual.id].setDimension = _extends({}, virtual), cyre.cyre.call('snap to position', virtual);
  }); //snap to grid
  //manages container

  cyre.cyre.on('SNAP', function (virtual) {
  ...
  var init = function init(au) {
    ...
    cyre.cyre.on('SHAKE', _addShake); //setup screen on initiation
